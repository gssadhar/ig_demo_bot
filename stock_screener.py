...
def fetch_uk_market_signals():
    """Expanded UK Equities Universe integrated with Front-Run Structural Pivot/Resistance Targets."""
    uk_data = [
...
def fetch_us_market_signals():
    """Expanded USA Equities Universe integrated with Front-Run Structural Pivot/Resistance Targets."""
    us_data = [
...
def execute_strong_buys(df, market, ig_service):
    ...
    for _, row in df.iterrows():
        if row["SIGNAL"] == "STRONG BUY":
            ...
            try:
                # Step 1: Execute clean market order with 0.5 lot size
                ...
                if response and response.get("dealStatus") == "ACCEPTED":
                    ...
                    # Step 2: Attach final macro target and stop loss
...
def monitor_and_manage_runners(df_combined, ig_service):
    """
    Hybrid Structural Exit Management:
    - Monitors price against structural pivot/resistance target (TARGET_1_2P5).
    - When price reaches resistance, closes 50% (0.25 size) to lock profits.
    - Leaves remaining 0.25 size running toward 1:4 macro target.
    """
    if not ig_service:
        return
...
def main():
    print("--- Starting 4x Daily Runner Strategy Pipeline (Structural Pivot Integration) ---")
    ...
    # 1. Manage active runner positions against structural resistance
    monitor_and_manage_runners(combined_df, ig_service)
    
    # 2. Execute new entry signals with 0.5 lot size
